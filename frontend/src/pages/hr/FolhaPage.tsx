...
function fmtPct(v: any) { return (Number(v??0)*100).toFixed(2)+"%" ; }
...
export default function FolhaPage() {
  ...
  async function handleNova() {
    setSaving(true);
    try {
      await api.post("/hr/folha",{competencia});
      setShowNova(false); load();
  ...
  async function handleCalcular(id: string) {
    try {
      await api.post("/hr/folha/"+id+"/calcular");
      load(); if(selected?.id===id) loadDetalhe(id);
  ...
  async function handleFechar(id: string) {
    ...
    try {
      await api.patch("/hr/folha/"+id+"/fechar");
      load(); if(selected?.id===id) loadDetalhe(id);
  ...
  async function handleReabrir(id: string) {
    try {
      await api.patch("/hr/folha/"+id+"/reabrir");
      load(); if(selected?.id===id) loadDetalhe(id);
    } catch(e:any){ alert(e?.response?.data?.message??"Erro"); }
  }

  function selFolha(f: any) {
    setSelected(f); setDetalhe(null); loadDetalhe(f.id);
  }
      ...
      <div style={{width:320,borderRight:"0.5px solid #E5E7EB",display:"flex",flexDirection:"column",flexShrink:0}}>
        ...
        <div style={{flex:1,overflow:"auto"}}>
          {loading ? <div style={{padding:40,textAlign:"center",color:"#9CA3AF"}}>Carregando...</div> :
            folhas.length===0 ? <div style={{padding:40,textAlign:"center",color:"#9CA3AF",fontSize:13}}>Nenhuma folha cadastrada.</div> :
            folhas.map(f=>{
              ...
              return (
                <div key={f.id} onClick={()=>selFolha(f)}
                  ...
                  <div style={{fontSize:12,color:"#6B7280",marginTop:4}}>
                    {f._count?.funcionarios??0} funcionario(s) · {fmtBRL(f.totalLiquido)} liquido
                  </div>
                </div>
              );
            })
          ...
          <div style={{padding:24}}>
            ...
            <div style={{background:"#fff",borderRadius:12,border:"0.5px solid #E5E7EB",overflow:"hidden"}}>
              <table style={{width:"100%",borderCollapse:"collapse"}}>
                <thead>
                  <tr>{["Funcionario","Salario Base","Bruto","INSS Emp.","INSS Pat.","IRRF","FGTS","Descontos","Liquido"].map(h=>(
