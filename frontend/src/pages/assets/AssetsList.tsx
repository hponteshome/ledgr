    ...
    async function bulkAction(action: 'activate' | 'deactivate' | 'reactivate' | 'delete' | 'backfill') {
        ...
        try {
            if (action === 'backfill') {
                ...
                for (const id of ids) {
                    try {
                        await fetch(`${API}/assets/${id}/depreciation/backfill`, { method: 'POST', headers, body: '{}' });
                        ok++;
                    } catch { fail++; }
                }
            } else {
        ...
        <div className="p-6 space-y-4">
            ...
            {someSelected && (
                <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-2.5 flex items-center gap-3 flex-wrap">
                    ...
                    <button onClick={() => bulkAction('backfill')} disabled={bulkLoading}
                    ...
                    {bulkLoading && <span className="text-xs text-blue-500 animate-pulse">Processando...</span>}
