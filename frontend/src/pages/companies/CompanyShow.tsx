      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                <span className={`text-[10px] font-black px-2 py-0.5 rounded uppercase ${company.status === 'active' || company.status === 'ativa' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                  ● {company.status}
                </span>
              </div>
            </div>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => navigate(`/app/companies/edit/${id}`)}
              className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition font-bold text-xs uppercase shadow-sm">
              <FiEdit2 size={13} /> Editar
            </button>
            <button onClick={() => navigate('/app/companies')}
              className="flex items-center gap-2 px-4 py-2 bg-white text-gray-500 border border-gray-200 rounded-xl hover:bg-gray-50 transition font-bold text-xs uppercase shadow-sm">
              <FiArrowLeft /> Voltar
            </button>
          </div>
        </div>
              ...
              <Section title="Perfil da Unidade" color="border-blue-600">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-x-10 gap-y-5">
                  ...
                  <div><Label>Capital Social</Label>
                    <p className="font-bold text-blue-600 text-base">R$ {formatCurrency(company.equity || company.capitalSocial)}</p>
                  </div>
                  ...
                  <div className="col-span-full p-3 bg-gray-50 rounded-xl border border-gray-100">
                    ...
                    <p className="text-xs font-bold text-gray-600 leading-relaxed">{company.mainActivity || company.atividade_principal || '—'}</p>
                  </div>
                </div>
              </Section>
                  ...
                  {(company.partners || company.qsa || company.socios)?.length > 0 ? (
                    (company.partners || company.qsa || company.socios).map((p: any, i: number) => (
                      <div key={i} className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                        <p className="text-[10px] font-black text-gray-700 uppercase leading-tight">{p.nome || p.name || 'Sócio não identificado'}</p>
                        <p className="text-[9px] font-bold text-purple-500 uppercase">{p.qualificacao || p.role || 'Sócio'}</p>
                      </div>
                    ))
                  ) : (
                    <p className="text-[10px] font-bold text-gray-400 uppercase italic text-center py-4">Nenhum sócio registrado</p>
                  )}
                </div>
              </Section>
            </div>
        ...
        {activeTab === 'contabil' && (
          <div className="space-y-6">
            <Section title="Contador Responsável" color="border-blue-600">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="md:col-span-2"><Inp label="Nome" value={config.accountantName} onChange={v => upd('accountantName', v)} /></div>
                <Inp label="Cargo" value={config.accountantRole} onChange={v => upd('accountantRole', v)} />
            ...
            <Section title="Responsável Legal" color="border-purple-600">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="md:col-span-2"><Inp label="Nome" value={config.legalRepName} onChange={v => upd('legalRepName', v)} /></div>
                <Inp label="Cargo" value={config.legalRepRole} onChange={v => upd('legalRepRole', v)} />
                <Inp label="CPF" value={config.legalRepCpf} onChange={v => upd('legalRepCpf', v)} mono />
              </div>
            </Section>

            <Section title="Responsável eSocial" color="border-orange-500">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="md:col-span-2"><Inp label="Nome" value={config.esocialName} onChange={v => upd('esocialName', v)} /></div>
                <Inp label="Cargo" value={config.esocialRole} onChange={v => upd('esocialRole', v)} />
                <Inp label="CPF" value={config.esocialCpf} onChange={v => upd('esocialCpf', v)} mono />
              </div>
            </Section>
            ...
            <Section title="Auditor Independente" color="border-gray-400">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <Inp label="Cargo" value={config.auditorRole} onChange={v => upd('auditorRole', v)} />
            ...
            <Section title="Dados ECD / ECF" color="border-green-600">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <Inp label="Código SCP" value={config.codigoScp} onChange={v => upd('codigoScp', v)} mono />
            ...
            <div className="flex items-center justify-between bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
              ...
              <div className="ml-auto">
                <button onClick={saveConfig} disabled={configSaving}
                  ...
                  <FiSave size={13} /> {configSaving ? 'Salvando...' : 'Salvar Configuração'}
                </button>
              </div>
            </div>
          </div>
