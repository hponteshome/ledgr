...
## 2026-09-20 13:47 - Performance dos balancetes, tipo Compensacao, layout do Livro LALUR e Reverter da Equivalencia [PERF-COMPENSACAO-LAYOUT-EQUITY-2026-09-20]
- Performance (trial-balance.service): getMovements deixou de trazer TODAS as partidas do intervalo para a memória do Node (findMany + soma em JS) e passou a agregar no banco (groupBy por accountId+type, _sum.value). Mesmo mapa de débitos/créditos por conta (só muda de onde vem a soma). A lista de Encerramento chamava getVerificationBalance 2x por ano (acumulado desde 1900) para cada exercício; o ganho vale também para Balancete, DRE, Balanço e Equivalência. Ganho de tempo a acompanhar em uso.
- Compensação (Apuração de Impostos, aba LALUR): removido o tipo "Compensação" do formulário de itens (com aviso de que a compensação de prejuízos fiscais é calculada na Parte B, limite de 30% do lucro real) e o addLalurItem passou a recusar tipo COMPENSACAO. Itens antigos continuam visíveis; o campo "compensações" do cálculo mensal de IRPJ/CSLL é outro e não mudou.
- Livro LALUR (LivroLalurPage): barra superior fixa (sticky top 12, zIndex 40, mesma sombra da ReportToolbar); cards-resumo IRPJ e CSLL do exercício (saldo inicial, novo prejuízo, compensação, saldo final, lucro real do ano); Parte B em abas IRPJ | CSLL com mini-gráfico do saldo final por ano, ano selecionado em destaque, linhas sem movimento esmaecidas e coluna "Lucro real do ano"; cabeçalhos das tabelas fixos. Lógica, endpoints e impressão inalterados.
- Equivalência Patrimonial (EquityMethodPage): botão "Reverter" em cada apuração do histórico (POST /accounting/equity-method/:id/reverter com referenceDate; exclui o lançamento de ajuste e o registro da apuração); confirmação com aviso quando há apurações posteriores (não são recalculadas); data-base exibida em UTC (a coluna é date-only e aparecia um dia antes).
- Pendências resolvidas nesta rodada: lentidão da lista de Encerramento (acompanhar), tipo Compensação e botão Reverter Cálculo do front.
- Pendências restantes: getReceitasBrutas (PIS/COFINS) sem filtro de deletedAt/isClosingEntry e com datas locais; validador do importador manual (detectar deslocamento de colunas com mensagem única e fechar o grupo ao mudar de data).

## 2026-09-20 14:02 - Receita bruta (PIS/COFINS) e validador do importador manual [RECEITAS-BRUTAS-IMPORTADOR-2026-09-20]
- apuracao.service.getReceitasBrutas: receita bruta = movimento real das contas de receita (crédito − débito por conta, via groupBy accountId+type), só lançamentos ativos (deletedAt null) e sem encerramento (isClosingEntry false), datas em UTC. Antes somava o value de todas as partidas (débito+crédito juntos), incluía excluídos e encerramentos (o encerramento debita a receita e entrava somando) e usava Date local. Mesmo formato de retorno ({ total, itens }); corrige o PIS/COFINS e a receita usada na presunção do IRPJ/CSLL (Lucro Presumido). Apurações já gravadas só mudam ao recalcular a competência.
- journal-manual-import.service (parseManualFile): (1) colunas deslocadas - NrLancto (2ª coluna, deve ficar vazia) com código de conta (3+ dígitos), Débito preenchido e Crédito vazio: o arquivo é recusado com UMA mensagem (linhas afetadas + formato correto Data||Débito|Crédito|Histórico|HP|Complemento|Valor), em vez de dezenas de erros de grupo; NrLancto curto (sequência) ou com Crédito preenchido não dispara. (2) Grupo de partida dobrada aberto e data diferente: um erro por grupo (linhas, data, débito ≠ crédito), o grupo é descartado e a linha atual abre um novo (sem erro em cascata).
- Aprendizado: o padrão "somar value ignorando type + sem deletedAt/isClosingEntry + datas locais" apareceu em getResultadoContabil e getReceitasBrutas. Toda consulta nova de movimento deve usar groupBy accountId+type, deletedAt null, isClosingEntry false e datas em UTC.
- Lista de pendências da nota de 20/09 ZERADA (trava do encerramento, lentidão, Compensação, Reverter Cálculo, getReceitasBrutas, validador do importador). Em aberto só testes em uso: duas abas no Encerramento (só 1 par ativo), tempo da lista de Encerramento, recalcular competências de PIS/COFINS/Presumido já apuradas e validar o arquivo deslocado no importador.

## 2026-09-20 14:16 - Sunrise: hard delete do Plano de Contas e reimportação do Plano Matriz [SUNRISE-RESET-PLANO-2026-09-20]
- Operação (20/09/2026): hard delete de TODO o Plano de Contas da Sunrise (company ad8ad459-40b4-47f5-8a9a-7e4688487b4f) e reimportação do Plano Matriz (420 contas com pai, mais as raízes). Journal entries e ativos não foram tocados (0 dependências).
- Método: dependências descobertas pelo catálogo do Postgres (pg_constraint: FKs -> chart_of_accounts) com contagem por tabela para a empresa via query_to_xml (dry-run somente leitura). Na Sunrise havia dependentes em account_balances (291), chart_of_accounts_ecd_imports (77), ecd_account_mappings (35 source + 35 target) e equity_method_investments (1 participação, com as 3 contas); journal_entry_items e demais tabelas = 0.
- Script: backup pg_dump -Fc (docker exec + docker cp para D:\Temp) -> transação única (BEGIN/COMMIT, psql -v ON_ERROR_STOP=1) com checagens dinâmicas que abortam se surgir dependência nova -> exclusão na ordem equity_method_investments, chart_of_accounts_ecd_imports, ecd_account_mappings, account_balances, zerar parent_id, chart_of_accounts -> verificação de 0 contas restantes. O arquivo .sql tem o ID da empresa fixo: renomear para .USADO.sql depois de usar (rodar de novo apagaria o plano novo).
- Efeitos colaterais: de/para ECD e vínculos ECD da Sunrise apagados (refazer De/Para se necessário); participação de Equivalência precisa ser recadastrada; company_accounting_config guarda IDs de contas SEM FK (encerramento: apuração/lucro/prejuízo) e fica apontando para contas inexistentes até reconfigurar na aba Contábil da empresa.
- Aprendizado: antes de hard delete de plano de contas, rodar a consulta de dependências por FK (dry-run) e só então gerar o script; a lista de tabelas muda com o tempo, por isso as checagens dinâmicas dentro da transação.

## 2026-09-20 15:03 - Config contábil: contas de saldo anterior (Lucros/Prejuízos Acumulados) e Sunrise refeita em 2017 [CONFIG-CONTAS-ACUMULADAS-SUNRISE-2026-09-20]
- Config contábil (aba Contábil > Encerramento de Exercício): dois campos novos, OPCIONAIS - "Lucros Acumulados (saldo anterior)" e "Prejuízos Acumulados (saldo anterior)". Colunas encerramento_conta_lucros_acumulados_id e encerramento_conta_prejuizos_acumulados_id (uuid, nullable) em company_accounting_configs; migração manual prisma/migrations-manuais/20260920_config_contas_acumuladas.sql (docker cp/exec + prisma generate). Endpoint PUT /accounting/config sem alteração (upsertConfig grava ...dto). Ao limpar um dos campos novos a tela envia null (string vazia em coluna uuid dá erro).
- LALUR (apuracao.service, getSaldoAberturaPrejuizo): o saldo anterior de prejuízos é lido da conta de Prejuízos Acumulados quando preenchida; sem ela, cai na conta de Prejuízo do Exercício (comportamento anterior). Leitura exata na conta indicada (a ideia de somar "contas irmãs" do mesmo pai foi descartada). Lucros Acumulados é salvo mas nenhum cálculo o usa ainda.
- Motivo: na Hotelsys a conta configurada como Prejuízo do Exercício é a 23301020001 (Prejuízos Acumulados), que recebe encerramentos e o saldo de abertura; na Sunrise o encerramento lança em 23301020002 (Prejuízo do Exercício = resultado corrente) e o saldo anterior fica em 23301020001 (Prejuízos Acumulados), então o LALUR vinha zerado. Recomendação: preencher Prejuízos Acumulados nas duas empresas com 23301020001.
- Sunrise (continuação da nota do hard delete): o delete por company_id levou também as 77 contas ECD_NATIVE e os vínculos ECD - a ECD 2017 (ECDSunrise2017.txt) foi reimportada; removido o registro antigo de ecd_imports (descrevia importação sem contas; cópia em D:\Temp\ecd_imports_sunrise.csv); De/Para de 31/12/2017 refeito (ECD 23501002 PREJUIZOS ACUMULADOS -> Matriz 23301020001) e Lançamento de Abertura refeito, com 25.248.344,65 D em 23301020001; o LALUR da Sunrise passou a ler o saldo (validado pelo usuário). Config de encerramento da Sunrise conferida: apuração 23401010001, lucro 23301010002, prejuízo 23301020002 (todas analíticas de PL da empresa).
- A ECD da Sunrise acusa divergência I155 x I250 em 6 contas (ex.: 23501002: I155 25.248.344,65 x soma dos I250 14.858.726,34) - vem do arquivo, não do sistema. ecd_account_mappings_bkp1426 é tabela avulsa (fora do schema) com IDs de contas antigas: não serve para restaurar o De/Para.
- Aprendizado: ao apagar o "plano de contas" de uma empresa, decidir pela origem (MATRIZ x ECD_NATIVE) - apagar por company_id remove também o histórico ECD preservado e obriga a reimportar a ECD.
- Testes em uso ainda em aberto: duas abas no Encerramento (só 1 par ativo, SQL count = 2), tempo da lista de Encerramento, recalcular competências de PIS/COFINS/Presumido já apuradas, validar o arquivo deslocado no importador manual e conferir o novo visual do Livro LALUR (barra fixa, cards, abas IRPJ/CSLL).

## 2026-09-20 15:06 - Baixa de pendências confirmadas em uso [BAIXA-PENDENCIAS-2026-09-20-B]
- Confirmado em uso pelo usuário (20/09/2026) e BAIXADO da lista de testes em aberto: (2) tempo da lista de Encerramento após o groupBy em getMovements, com os mesmos valores por ano; (5) novo visual do Livro LALUR (barra fixa, cards-resumo IRPJ/CSLL, abas IRPJ | CSLL e mini-gráfico da Parte B).
- Seguem em aberto (só conferência em uso, sem código novo): (1) duas abas no Encerramento - dois "Confirmar" quase simultâneos no mesmo ano aberto devem deixar só 1 par ativo (SQL count = 2); (3) recalcular as competências de PIS/COFINS/Presumido já apuradas e comparar a receita bruta com a da DRE do mês; (4) validar o arquivo deslocado no importador manual (mensagem única de layout).

## 2026-09-21 - Auditoria de integridade e efetividade do Backup [AUDITORIA-BACKUP-2026-09-21]
- GitHub: origin/main == HEAD (109b21f), 0 commits sem push, 0 alteracoes pendentes, sem branch/stash local, sem .git aninhado, nenhum .env/dump/certificado versionado. Confirmar que o repositorio hponteshome/ledgr e PRIVADO (docker-compose.yml tem credenciais em texto claro).
- Cobertura: pg_dump sem -t/-T; 124 tabelas no banco = 124 com TABLE DATA no dump. Teste de restauracao real (banco temporario ledgr_restoretest): pg_restore exit 0, contagem de linhas identica nas 124 tabelas (207.558 linhas; 60 tabelas vazias no original).
- Fora do banco e do GitHub (antes sem cobertura): uploads/, apps/api/uploads/, LM/ (arquivos-fonte ECD/IOB, ~50 MB) e os .env. Agora cobertos pelo script.
- backup-postgres.ps1 reescrito: checa exit code de pg_dump/docker cp/tar; valida cada dump (pg_restore -l, tabelas do dump x banco) e falha se divergir; container parado agora sai com exit 2 (antes exit 0 silencioso); log em D:\Backups\ledgr-postgres\backup.log; rotacao nunca apaga os 48 dumps / 7 zips mais recentes; 1x por dia gera arquivos_*.zip (uploads, LM) e segredos_*.zip (.env) em D:\Backups\ledgr-arquivos; variavel $secondaryDir (vazia = desativada) para copia secundaria (dump e arquivos, NUNCA os segredos).
- Tarefa Agendada LEDGR-Postgres-Backup (Ready, S4U) segue chamando o mesmo caminho do script - sem recriar.
- PENDENCIAS: (1) definir destino secundario FORA deste PC (HD externo/nuvem/SERVER02) e preencher $secondaryDir - hoje todas as copias estao no mesmo computador; (2) guardar copia dos .env em gerenciador de senhas ou midia criptografada; (3) confirmar repositorio GitHub privado; (4) repetir o teste de restauracao periodicamente (ex.: mensal); (5) entender se uploads/ contem certificados/assinaturas e se a chave que os protege esta nos .env.

## 2026-09-21 - ECD Razao (consulta) independente do Contabil [ECD-ARQUIVO-FIEL-2026-09-21]
- Objetivo: consultar razao SO com dados das ECDs (Hotelsys 2015-2025), com o plano de contas de cada ano, sem tocar na contabilidade.
- Arquitetura: 6 tabelas novas (ecd_arquivos, ecd_arq_contas, ecd_arq_saldos, ecd_arq_lancamentos, ecd_arq_partidas, ecd_arq_historicos). Migracao aditiva prisma/migrations-manuais/20260921_ecd_arquivo_fiel.sql (rollback no cabecalho). Modelos EcdArq* APENDADOS ao schema.prisma, sem relacao Prisma com Company (FK so no SQL).
- Backend: apps/api/src/modules/sped/ecd-arquivo/ (parser puro ecd-arq-parser.ts, service, controller, module), registrado no SpedModule (2 linhas). Rotas: POST sped/ecd-arquivo/carregar (unica que grava, so em ecd_arq*), GET arquivos, :id/contas, :id/razao, :id/razao-geral (varias contas / intervalo / geral; limite 20000 partidas).
- Frontend: pages/sped/EcdArquivoPage.tsx, rota /app/sped/ecd/arquivo (routes/index.tsx +2 linhas). Uma conta ou varias/intervalo/geral, conferencia com o I155, CSV (separador |) e impressao A4 paisagem.
- Menu: 1 linha em sidebar_items (path /app/sped/ecd/arquivo), autorizada pelo usuario - prisma/migrations-manuais/20260921_sidebar_ecd_razao.sql (rollback: DELETE FROM sidebar_items WHERE path = /app/sped/ecd/arquivo). Permissoes por perfil/usuario NAO foram tocadas: liberar o item para outros perfis na tela de permissoes.
- Carga: os .txt sao lidos em memoria (nada e gravado em disco). Hotelsys 2015..2025 carregadas (11 arquivos): lancamentos e partidas conferem com os arquivos; debito = credito em todos; debitos dos lancamentos tipo N de 2018-2025 batem ao centavo com o Contabil (ECD_IMPORT).
- Prova de independencia: retrato (contagem + hash de conteudo) das 124 tabelas existentes antes/depois da carga (D:\Temp\snap_tabelas.ps1). Unicas diferencas: sidebar_items +1 (INSERT autorizado) e menu_usage_stats (contador de uso do proprio app).
- ACHADOS NAO CORRIGIDOS (fora do escopo; nao mexer no Contabil sem decisao): (1) EcdParserService mistura o Bloco C (dados recuperados da ECD anterior) com o Bloco I - por isso a tela ECD mostra 360 contas / 1346 saldos em 2025, quando o arquivo tem 180 / 1234 no bloco I; (2) o historico padrao esta no I075 e o parser le 0400 (vazio); I051/I052 sao ignorados; (3) o lancamento que falta por ano no Contabil (2018-2025) e o de encerramento (tipo E), causa nao confirmada; (4) a coluna PERIODO da tela ECD mostra 12/2024 -> 12/2025 para arquivo 01/2025-31/12/2025 (deslocamento de 1 mes na exibicao); (5) fiscal_obligations teve conteudo alterado durante a sessao sem relacao com esta funcionalidade - origem nao identificada.
- PENDENCIAS: validar na tela razao geral 2018-2025, intervalo, grupo e impressao; 2015-2017 excedem o limite no razao geral do ano inteiro (usar contas/periodo); avaliar corrigir os achados (1) a (4) em sessao propria; liberar o menu para outros perfis se necessario.

## 2026-09-23 - ECF Arquivo Fiel + Analise do LALUR + conferencia ECD x ECF [ECF-ARQUIVO-FIEL-2026-09-23]
- Objetivo: replicar o modelo da ECD (arquivo fiel, somente leitura, independente do Contabil) para a ECF, com analise profunda do LALUR (Hotelsys).
- Arquitetura: 3 tabelas novas (ecf_arquivos, ecf_arq_periodos, ecf_arq_registros - guarda linha a linha, campos como text[], pois os registros do leiaute mudam muito entre versoes 0004-0012). Migracao prisma/migrations-manuais/20260921_ecf_arquivo_fiel.sql. Regra de versao vigente por empresa+periodo: retificadora sempre vence a original; copias com acentuacao danificada perdem para a integra (campo qualidade); conflitos reais (blocos M/N/L/P divergentes) sao rejeitados e nada e alterado.
- Backend: apps/api/src/modules/sped/ecf-arquivo/ (parser puro ecf-arq-parser.ts, service, controller, module), registrado no SpedModule. Rotas somente leitura: arquivos, lalur/resumo, lalur/cadeia, lalur/parte-a-validacao, lalur/parte-b-validacao, :id/parte-a, :id/parte-b, :id/indice, :id/registros. Unica rota que grava: POST carregar (so em ecf_arq*).
- Frontend: pages/sped/EcfArquivoPage.tsx (~1700 linhas), rota /app/sped/ecf/arquivo, 6 abas: Visao geral (resumo do LALUR por ano + cadeia da Parte B IRPJ/CSLL + validacao completa Parte A/B), Parte A, Parte B, Registros, ECD x ECF, Inconsistencias (lista consolidada de todos os achados com link "Ver detalhe" para a aba de origem).
- Menu: 1 linha em sidebar_items (prisma/migrations-manuais/20260921_sidebar_ecf_lalur.sql), autorizada pelo usuario.
- ECD arquivo (ajuste feito nesta sessao): filtro do "Resultado do Exercicio" corrigido para exigir ind_cta=A (existia uma conta sintetica homonima "6.1.01" que causava "conta ambigua" de 2018 em diante); endpoint GET resultado-anual novo, usado na conferencia ECD x ECF; efeito de busca corrigido para carregar independente da aba ativa (antes so buscava ao visitar Visao Geral, causando falsos "Falta ECD" nas demais abas).
- BUGS REAIS CORRIGIDOS DURANTE A ANALISE (nao sao achados de auditoria, eram erros do codigo da ferramenta):
  1. Parte B "quebra": comparava o saldo inicial de cada conta contra o ULTIMO fechamento historico (qualquer ano passado) em vez de contra o fechamento do ano IMEDIATAMENTE anterior - gerava falsas quebras quando uma conta reaparecia anos depois com valor residual. Corrigido: comparacao restrita a fechamentosAnoAnterior.
  2. Parte A "consistencia": comparava M300 codigo 2 (lucro ANTES do IRPJ/CSLL) contra L300 codigo 3 (resultado FINAL, ja liquido de impostos) em vez de L300 codigo 3.01 (tambem antes do IRPJ/CSLL) - gerava falsas divergencias em anos com provisao contabil de imposto (2014, 2016, 2017). Corrigido: compara contra 3.01.
- ACHADOS REAIS CONFIRMADOS (auditoria de verdade, nao bug):
  1. 2020 (retificadora vigente): R$ 20.682.933,69 de exclusao no IRPJ/CSLL (codigo 166.35, SEM LASTRO CONTABIL - nenhuma conta M310/M360 por tras), com historico do proprio contador citando "despesas dedutiveis dos anos de 2018 e 2019". Absorve o efeito da lacuna dessas duas ECFs, que nao existem.
  2. Parte B fecha perfeitamente de 2014 a 2024 (IRPJ e CSLL), conta a conta, inclusive atravessando a troca de conta 24204999 -> 24404999 em 2021. Unica quebra real remanescente: 2025 (original) nao herdou a Parte B da retificadora de 2024, e sim da ECF 2024 original - RETIFICACAO EM ANDAMENTO (2025) para corrigir.
  3. 2017: SOMENTE a linha "Resultado do exercicio (ECD x ECF)" e divergencia real (R$ 4.137.643,95), explicada por uma exclusao de mesmo valor no codigo 167.01 do M300, tambem SEM LASTRO CONTABIL. Memoria de calculo dessa exclusao ainda nao localizada - proximo passo da analise de 2017.
  4. Nunca houve compensacao de prejuizo fiscal/base negativa em nenhum ano (M300 169 sempre igual a 171): trava de 30% nunca testada.
  5. 2015: exclusao de R$ 9.696.969,69 so na CSLL (167.01), sem par no IRPJ - a explicar (legitima ou lancada no tributo errado).
- PENDENCIAS: concluir retificacao da ECF 2025; localizar memoria de calculo da exclusao de 2017; investigar a defasagem de reconhecimento ECD x ECF em 2015/2016 (resultado da ECD aparece ~1 ano deslocado); decidir sobre a exclusao exclusiva de CSLL em 2015; Sunrise ainda nao coberta pela analise (so ECF 2017 carregada, sem ECD).

## 2026-09-23 - Reclassificacao de Acumulados no encerramento + Competencia Ativa nos relatorios [ENCERRAMENTO-COMPETENCIA-2026-09-23]
- Achado (Sunsys, revisao do Balanco Patrimonial 2025): a conta "Prejuizo do Exercicio" (23301020002) acumulava o resultado de TODOS os 8 encerramentos ja rodados (2018-2025, somando R$ 13.749.253,30), porque encerramento-exercicio.service.ts sempre lancava o resultado do ano na MESMA conta fixa (config.encerramentoContaLucroExercicioId/PrejuizoExercicioId), sem nunca zera-la no encerramento seguinte - a conta que deveria mostrar so o resultado do ANO ficava com o acumulado historico. Valores identicos em 2019-2023 (R$ 1.777.916,64 cada) conferidos pelo usuario: unico movimento contabilizado nesses anos e a depreciacao do mesmo imovel, coincidencia legitima, nao erro.
- Correcao: nova "Etapa 1/3" no encerramento, ANTES de lancar o resultado do ano - calcula o saldo de Lucro/Prejuizo do Exercicio em 31/12 do ano ANTERIOR e reclassifica para as contas de Lucros/Prejuizos Acumulados (campos encerramentoContaLucrosAcumuladosId/PrejuizosAcumuladosId, ja existiam no schema e na aba Contabil - ContabilTab.tsx ja tinha os seletores "Lucros/Prejuizos Acumulados (saldo anterior)" - so nunca eram lidos pelo servico). So roda quando ha saldo a mover (primeiro encerramento nao gera lancamento); bloqueia ANTES de gravar qualquer coisa se faltar configurar as contas de Acumulados. Reverter continua funcionando sem alteracao (a nova etapa e datada no mesmo dia do fechamento, isClosingEntry=true, mesmo filtro do reverterSemTrava ja existente).
- Sunsys: usuario reverteu e reconfirmou TODOS os anos de encerramento apos o fix - sucesso confirmado. Config da Sunsys ja tinha as 2 contas de Acumulados cadastradas (23301010001 Lucros Acumulados / 23301020001 Prejuizos Acumulados).
- Competencia Ativa: achado sistemico em 7 telas de relatorio - nenhuma lia activeCompetencia do CompanyContext (persistida por usuario+empresa no backend, controlada pelo seletor "Mes/Ano Ativo" no Header.tsx). 4 telas (DrePage, BalancoPatrimonialPage, RazaoAnaliticoPage, DiarioGeralPage) tinham uma funcao getActiveYear() duplicada em cada arquivo lendo localStorage['@ledgr:activeCompetencia:ID'] - chave morta, nada mais escreve nela desde que a competencia passou a ser persistida no backend, sempre caia no fallback do ano corrente. TrialBalanceView.tsx (Balancete) tinha datas fixas no codigo (useState('31/12/2026')). LivroLalurPage.tsx e ApuracaoImpostosPage.tsx (esta ultima nem importava useCompany - usa window.__companyId__ para o companyId) usavam new Date().getFullYear() direto.
- Correcao: as 7 telas passam a ler activeCompetencia de useCompany() e usa-la como padrao inicial (ano ou DD/MM/AAAA conforme a tela), com fallback para o ano corrente so quando a competencia ainda nao foi definida. Risco residual assumido conscientemente: como cada pagina le activeCompetencia so no primeiro render (useState/useState lazy init), se o carregamento da competencia no CompanyContext for assincrono e mais lento que a primeira renderizacao da pagina, o padrao pode cair no ano corrente mesmo assim - nao construida protecao extra (useEffect de resync) para nao aumentar o risco de uma mudanca em 7 arquivos de uma vez; tratar caso a caso se o sintoma aparecer.
- PENDENCIAS: validar em uso real se alguma das 7 telas ainda abre no ano corrente (risco residual acima); nenhuma migracao de banco foi necessaria (campos de Acumulados ja existiam no schema).

## 2026-09-24 - Hard delete F5/Sunrise + fixes AccountTree + bug real na Competencia Ativa [SESSAO-2026-09-24-TARDE]
- **Hard delete F5 Participacoes (e274dfc0-0a9a-4af1-8141-f35ff73fac93):** 414 contas (origem MATRIZ, 0 lancamentos/saldos/ECD/vinculos em qualquer tabela) apagadas a pedido do usuario, para reimportar do zero. Migracao `prisma/migrations-manuais/20260924_hard_delete_plano_contas_f5.sql`, com guarda de contagem (414) antes de apagar. Prova por retrato de tabelas: so chart_of_accounts mudou. Reimportado Plano Matriz (516 contas) em seguida, sem incidente (bug de colisao de codigo documentado em 18/09 nao se aplica a plano vazio).
- **Hard delete completo da Sunrise Holding (ad8ad459-40b4-47f5-8a9a-7e4688487b4f):** 482 contas + 291 account_balances + 1 ecd_import + 28 ecd_account_mappings + 77 chart_of_accounts_ecd_imports apagados em cascata (migracao `prisma/migrations-manuais/20260924_hard_delete_contabilidade_sunrise.sql`), para reimportar Matriz + ECD 2017 + ECF do zero. As 5 contas de encerramento configuradas em company_accounting_configs foram zeradas junto (ficariam apontando para contas apagadas). Reimportado: Matriz + ECD 2017 (60 lancamentos, 159 itens) + Lancamento de Abertura ABERTURA-2018 (31/12/2017, D=C=R$25.249.787,92, 12 contas, confirmado sem faltar nenhuma conta com saldo nao-zero em 31/12/2017 - achado inicial de "lancamentos espalhados pelo ano" era falso alarme, era so o unico lancamento de abertura aparecendo dentro do periodo do Balancete).
- **AccountTree.tsx, 2 fixes de UI:**
  1. `fmtDiff()`: quando a diferenca ECD x calculado e ~0, renderizava a PALAVRA "check" (texto literal, nao um icone) - trocado pelo simbolo ✓, consistente com a legenda "✓ = sem divergencia" ja existente em AccountsPage.tsx.
  2. Colunas Saldo Calculado/Saldo ECD/Diferenca sem `overflow:hidden` - quando a largura salva no navegador (redimensionamento por arrasto) ficava estreita, o texto vazava visualmente por cima da coluna vizinha (ex: "1.443,27-1.592.129,00" grudados). Adicionado `truncate` nas 3 celulas.
  3. Pendencia NAO resolvida: coluna AÇÕES as vezes fica cortada da area visivel mesmo apos "Restaurar larguras das colunas" - usuario decidiu deixar pra depois (limite de sessao).
- **BUG REAL corrigido - Competencia Ativa nunca persistia entre trocas de empresa:** `company.service.ts` `setActiveCompetencia()` usava `.update()` do Prisma, que falha (P2025) quando o usuario nao tem linha propria em `user_companies` para aquela empresa - caso do proprio usuario (Master Admin), que so tinha 1 linha em user_companies apesar de transitar por 10+ empresas na sessao. O erro morria num catch silencioso no frontend (`CompanyContext.tsx`), entao a tela "parecia" salvar (cache local atualizava na hora) mas nunca persistia de verdade - toda troca de empresa voltava pro mes corrente porque o GET nunca achava nada salvo. Corrigido: `.update()` -> `.upsert()` (cria o vinculo em user_companies na hora, se faltar). Confirmado por SQL direto apos o fix: Hotelsys e Sunsys gravaram active_competencia=2025-12-31 corretamente, linhas de user_companies criadas automaticamente.
- **Achado colateral, nao investigado:** ao excluir 11 lancamentos da Sunrise "pela tela" (Diario), nenhum ficou com `deleted_at` preenchido - sumiram da tabela por completo. Rotina de exclusao em lote do Diario pode estar fazendo hard-delete real em vez de soft-delete (regra do projeto exige soft-delete). Vale investigar em sessao futura.
- **PENDENCIAS:** investigar exclusao em lote do Diario (hard-delete suspeito, acima); coluna AÇÕES do AccountTree cortada da area visivel mesmo com larguras restauradas; Sunrise aguardando ECF (arquivo fiel independente, ainda vazio).
