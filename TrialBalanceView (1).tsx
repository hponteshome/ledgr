...
const TrialBalanceView: React.FC = () => {
    ...
    const setLevelAndExpand = (level: number) => {
        setMaxLevel(level);
        if (viewMode === 'monthly') {
            expandToLevel(monthlyItems, level);
            setMonthlyItems([...monthlyItems]);
        } else {
            expandToLevel(verificationItems, level);
            setVerificationItems([...verificationItems]);
        }
    };
    ...
    const renderMonthlyItem = (item: MonthlyBalanceItem, level = 0, levelCap = maxLevel): React.ReactNode => {
        const hasChildren = !!item.children?.length;
        const isGroup = hasChildren;
        const canExpand = item.expanded && level + 1 < levelCap;
        ...
        return (
            <React.Fragment key={item.account.id}>
                ...
                {canExpand && item.children?.map(child => renderMonthlyItem(child, level + 1, levelCap))}
            </React.Fragment>
        );
    };
    ...
    const renderVerificationItem = (item: VerificationBalanceItem, level = 0, levelCap = maxLevel): React.ReactNode => {
        const hasChildren = !!item.children?.length;
        const isGroup = hasChildren;
        ...
        return (
            <React.Fragment key={item.account.id}>
                ...
                {canExpand && item.children?.map(child => renderVerificationItem(child, level + 1, levelCap))}
            </React.Fragment>
        );
    };
                    ...
                    <div className="flex flex-wrap items-end gap-3">
                        ...
                        {hasData && (
                            <div className="flex items-center gap-2 ml-auto">
                                {/* Seletor de nível */}
                                <div className="flex items-center gap-1.5 px-3 py-1.5 border border-gray-200 rounded text-xs text-gray-500">
                                    <span className="mr-1">Nível:</span>
                                    {[1, 2, 3, 4].map(n => (
                                        <button
                                            key={n}
                                            onClick={() => setLevelAndExpand(n)}
