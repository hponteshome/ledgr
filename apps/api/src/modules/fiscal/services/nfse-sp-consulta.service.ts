// apps/api/src/modules/fiscal/services/nfse-sp-consulta.service.ts
...
const SP_WS_URL  = 'https://nfews.prefeitura.sp.gov.br/lotenfe.asmx'; // Nova URL v1+v2
const SP_WS_URL_OLD = 'https://nfe.prefeitura.sp.gov.br/ws/lotenfe.asmx'; // Legado
const SP_WS_URL_HOM = 'https://nfehomws.prefeitura.sp.gov.br/lotenfe.asmx';
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/"
  ...
  // ── Consulta NFS-e Emitidas (como Prestador) ──────────────────────────────
  private buildConsultaEmitidas(cnpj: string, pagina: number, dtInicio?: string, dtFim?: string): string {
    const periodoBlock = dtInicio && dtFim
      ? `<tc:PeriodoInicial>${dtInicio}</tc:PeriodoInicial>
         <tc:PeriodoFinal>${dtFim}</tc:PeriodoFinal>`
      : '';
    return `<p1:ConsultaNFeEmitidas
      xmlns:p1="http://www.prefeitura.sp.gov.br/nfe"
      xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
      xmlns:tc="http://www.prefeitura.sp.gov.br/nfe/tipos"
      Versao="1">
      ...
      ${periodoBlock}
      <tc:Pagina>${pagina}</tc:Pagina>
    </p1:ConsultaNFeEmitidas>`;
  }

  private buildSoapEmitidas(xmlInterno: string): string {
    return `<?xml version="1.0" encoding="UTF-8"?>
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/"
  xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  <soap:Body>
    <ConsultaNFeEmitidas xmlns="http://www.prefeitura.sp.gov.br/nfe/ws/">
      <VersaoSchema>1</VersaoSchema>
      <MensagemXML><![CDATA[${xmlInterno}]]></MensagemXML>
    </ConsultaNFeEmitidas>
  </soap:Body>
</soap:Envelope>`;
  }

  async consultarEmitidas(params: {
    companyId:   string;
    certId:      string;
    dtInicio?:   string;
    dtFim?:      string;
    ...
    const company = await this.prisma.company.findUniqueOrThrow({ where: { id: params.companyId } });
    const cnpj    = (company.taxId ?? '').replace(/\D/g,'');
    if (!cnpj) throw new BadRequestException('Empresa sem CNPJ cadastrado.');
    ...
    const todasNotas: any[] = [];
    let totalEncontradas = 0, importadas = 0, duplicatas = 0;
    const erros: string[] = [];

    for (let pag = 1; pag <= maxPags; pag++) {
      this.logger.log(`Consultando SP NFS-e emitidas ${cnpj} — página ${pag}`);

      const xml  = this.buildConsultaEmitidas(
        cnpj, pag,
        params.dtInicio?.replace(/-/g,''),
        params.dtFim?.replace(/-/g,''),
      );
      const soap = this.buildSoapEmitidas(xml);
      ...
      try {
        ...
        erros.push(`Página ${pag}: ${String(e?.response?.data ?? e.message).slice(0,200)}`);
        break;
      }

      const erro = this.extractErro(respData);
      if (erro) { if (pag > 1) break; erros.push(erro); break; }

      const notasXml = this.extractNotasFromResponse(respData);
      if (!notasXml.length) break;
      totalEncontradas += notasXml.length;

      if (params.importar !== false) {
        for (const notaXml of notasXml) {
          try {
            const parsed = this.parser.parseXml(`<ListaNfse>${notaXml}</ListaNfse>`, cnpj);
            if (!parsed.length) continue;
            const n = parsed[0];
            const dup = await this.prisma.fiscalDocument.findFirst({
            ...
            await this.prisma.fiscalDocument.create({
              data: {
                companyId: params.companyId,
                documentType: 'NFSE', documentNumber: n.numero,
                accessKey: (n.codigoVerificacao+'0'.repeat(44)).slice(0,44),
                issuerCnpj: n.prestadorCnpj, issuerName: n.prestadorNome,
                issueDate: new Date(n.dataEmissao+'T12:00:00Z'),
                dueDate:   new Date(n.dataEmissao+'T12:00:00Z'),
                competenceMonth: n.competencia,
                grossAmount: new Decimal(n.valorServicos), discountAmount: new Decimal(n.valorDeducoes),
                netAmount: new Decimal(net), irAmount: new Decimal(n.valorIr),
                pisAmount: new Decimal(n.valorPis), cofinsAmount: new Decimal(n.valorCofins),
                csllAmount: new Decimal(n.valorCsll), issAmount: new Decimal(n.valorIss),
                inssAmount: new Decimal(n.valorInss), integrationStatus: 'PENDING', status: 'RASCUNHO',
                notes: `Busca automática SP EMITIDAS | ISS retido: ${n.issRetido?'Sim':'Não'} | ${n.itemListaServico}`,
                createdById: params.userId,
              },
            });
            importadas++; todasNotas.push(n);
          } catch(e: any) { erros.push(e.message?.slice(0,100)); }
        }
      }
      if (notasXml.length < 50) break;
    }
    return { cnpj, tipo:'EMITIDAS', totalEncontradas, importadas, duplicatas, erros,
      periodo: { inicio: params.dtInicio, fim: params.dtFim } };
  }
}
